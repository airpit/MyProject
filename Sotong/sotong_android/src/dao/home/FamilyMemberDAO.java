...
public class FamilyMemberDAO implements Serializable{
			...
			String memberId, String memberPw, byte memberRole)
	{
		...
		try{
			String sql = "insert into family_member_tb values (?, ?, ?, ?, ?, ?, ?, null, ?, null, null, ?)";
	...
	{
		FamilyMemberVO fmVO = getMemberInfoById(id);
		if(fmVO == null) { return null; }
		else if(fmVO.getMemberPw().equals(pw)) { return fmVO;} // 비밀번호 일치
		else { //비번 불일치
			return null;
		}
	}
			...
			byte memberRole)
	{
		...
		PreparedStatement pstmt = null;
		try{
			String sql = "update family_member_tb set member_name=?, member_phone=?, member_email=?,member_pw=?, member_photo=?, member_nickname=?, member_color=?, member_birth=?, member_role=? where member_code = ?";
			pstmt = conn.prepareStatement(sql);
